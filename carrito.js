const platillosEnCarrito = JSON.parse(localStorage.getItem("platillos-en-carrito"));
console.log(platillosEnCarrito);
...
function cargarProductosCarrito(){
	...
	if (platillosEnCarrito && platillosEnCarrito.length > 0 ) {
		...
		platillosEnCarrito.forEach(producto => {
...
function eliminarDelCarrito(e){
	...
	platillosEnCarrito.splice(index, 1);
	cargarProductosCarrito();
...
function vaciarCarrito(){
	...
	Swal.fire({
 	 	title: "¿Estas Seguro?",
  		text: ` se van a borrar todos los platillos`,
  		icon: "question",
...
function actualizarTotal(){
	const totalCalculado = platillosEnCarrito.reduce((acc, producto)=> acc +(producto.precio * producto.cantidad), 0);
